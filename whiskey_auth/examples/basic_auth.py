...
@app.role("reader")
class ReaderRole:
    permissions = [Permissions.READ_POSTS]
...
async def admin_action(user: CurrentUser):
    ...
    return {
        "message": f"Admin action performed by {user.username}",
        "admin": True
    }
    ...
    for test_user in [reader, writer, admin]:
        ...
        try:
            post = await create_post(
                "Test Post",
                "Content",
                __auth_context__=auth_context
            )
            print(f"  ✅ Can create posts")
        except Exception as e:
            print(f"  ❌ Cannot create posts: {type(e).__name__}")
        
        # Test admin action (requires admin role)
        try:
            result = await admin_action(__auth_context__=auth_context)
            print(f"  ✅ Can perform admin actions")
        except Exception as e:
