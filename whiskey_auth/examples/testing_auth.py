...
async def create_article(title: str, user: CurrentUser) -> dict:
    ...
    return {"title": title, "author": user.username, "created": True}
...
async def delete_user(user_id: int, user: CurrentUser) -> dict:
    ...
    return {"deleted_user_id": user_id, "deleted_by": user.username}
...
async def edit_article(article_id: int, user: CurrentUser) -> dict:
    ...
    return {"article_id": article_id, "edited_by": user.username}
...
class TestAuthenticationFlow:
    ...
    def users(self):
        ...
        return {
            "alice": create_test_user(
                id=1, username="alice", password="alice123", permissions=["read"], roles=["reader"]
            ),
            ...
            "inactive": create_test_user(id=4, username="inactive", is_active=False),
        }
    ...
    async def test_authentication_failure(self, auth_provider):
        ...
        # Wrong username
        user = await auth_provider.authenticate(username="nonexistent", password="password")
        assert user is None

        # Wrong password
        user = await auth_provider.authenticate(username="alice", password="wrongpassword")
        assert user is None
...
class TestAuthContext:
    ...
    def test_auth_context_assertions(self):
        ...
        user = create_test_user(
            username="testuser", permissions=["read", "write"], roles=["editor"]
        )
...
if __name__ == "__main__":
    ...
    admin_user = create_test_user(
        username="admin", permissions=["read", "write", "admin"], roles=["admin"]
    )
