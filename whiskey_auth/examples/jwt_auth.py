...
class DatabaseJWTAuth(JWTAuthProvider):
    ...
    async def get_user_by_id(self, user_id: int) -> User | None:
        row = await self.db.fetch_one(self.queries.get_by_id, {"id": int(user_id)})
        if not row:
            return None
...
class AuthService:
    def __init__(
        self, password_auth: DatabasePasswordAuth, jwt_auth: DatabaseJWTAuth, hasher: PasswordHasher
    ):
    ...
    async def login(self, username: str, password: str) -> dict:
        ...
        user = await self.password_auth.authenticate(username=username, password=password)
...
async def update_profile(email: str, user: CurrentUser, db: Database) -> dict:
    ...
    await db.execute(
        SQL("UPDATE users SET email = :email WHERE id = :id"), {"email": email, "id": user.id}
    )

    return {"id": user.id, "username": user.username, "email": email, "updated": True}
...
@inject
async def main(
    auth_service: AuthService, jwt_auth: DatabaseJWTAuth, db: Database, queries: UserQueries
):
    ...
    alice = await auth_service.register("alice", "alice@example.com", "SecureP@ss123")
    print(f"✅ Registered user: {alice.username}")

    bob = await auth_service.register("bob", "bob@example.com", "AnotherP@ss456")
    ...
    api_headers = {"Authorization": f"Bearer {alice_access_token}"}
