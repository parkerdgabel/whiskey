...
class TestCreateAuthContext:
    ...
    async def test_create_authenticated_context(self):
        ...
        context = await create_auth_context(
            user=user, provider_name="test", metadata={"ip": "127.0.0.1"}
        )
...
class TestAuthenticationMiddleware:
    ...
    def create_http_scope(
        ...
        return {"type": "http", "headers": scope_headers, "query_string": query_string.encode()}

    @pytest.mark.asyncio
    async def test_extract_auth_from_header(self, middleware, registry):
        """Test extracting auth from Authorization header."""
        scope = self.create_http_scope(headers={"Authorization": "Bearer valid-token"})
    ...
    async def test_extract_auth_invalid_header(self, middleware):
        ...
        # Wrong prefix
        scope = self.create_http_scope(headers={"Authorization": "Basic sometoken"})

        auth_context = await middleware.extract_auth(scope)
        assert not auth_context.is_authenticated

        # Invalid token
        scope = self.create_http_scope(headers={"Authorization": "Bearer invalid-token"})
    ...
    def test_parse_cookies(self, middleware):
        ...
        assert cookies == {"session_id": "abc123", "user": "alice", "theme": "dark"}
    ...
    def test_parse_query_string(self, middleware):
        ...
        assert params == {"api_key": "secret", "user": "alice", "debug": "true"}
