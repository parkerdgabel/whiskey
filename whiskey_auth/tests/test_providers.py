...
class TestPasswordAuthProvider:
    ...
    def test_users(self):
        """Create test users."""
        return [
            create_test_user(id=1, username="alice", password="alice123", is_active=True),
            create_test_user(id=2, username="bob", password="bob123", is_active=False),
        ]
...
class TestJWTAuthProvider:
    ...
    def test_decode_token_invalid(self, provider):
        ...
        # Wrong issuer
        token = jwt.encode({"sub": "1", "iss": "wrong-issuer"}, "test-secret", algorithm="HS256")
        assert provider.decode_token(token) is None
    ...
    def test_refresh_with_invalid_token(self, provider):
        ...
        access_token = jwt.encode({"sub": "1", "type": "access"}, "test-secret", algorithm="HS256")
