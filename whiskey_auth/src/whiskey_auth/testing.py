        ...
        if username and password:
            for user in self.users:
                if user.username == username and (
                    (hasattr(user, "password") and user.password == password)
...
def test_user():
    ...
    return create_test_user(
        id=1, username="testuser", email="test@example.com", password="testpass123"
    )
...
def assert_authenticated(auth_context: AuthContext, user: User | None = None):
    ...
    if user:
        assert auth_context.user.id == user.id, (
            f"Expected user {user.id}, got {auth_context.user.id}"
        )
...
def assert_lacks_permission(auth_context: AuthContext, permission: str | Permission):
    ...
    Args:
        ...
        permission: Permission to check
    """
    assert not auth_context.has_permission(permission), (
        f"User should not have permission: {permission}"
    )
