...
class TestASGIManager:
    ...
    def test_add_route(self):
        ...
        route1 = RouteMetadata(func=lambda: "hello", path="/hello", methods=["GET"], name="hello")

        route2 = RouteMetadata(func=lambda: "world", path="/world", methods=["POST"], name="world")
    ...
    def test_add_websocket(self):
        ...
        ws1 = WebSocketMetadata(func=lambda ws: None, path="/ws", name="ws1")

        ws2 = WebSocketMetadata(func=lambda ws: None, path="/chat/{room}", name="ws2")
    ...
    def test_find_route(self):
        ...
        route1 = RouteMetadata(
            func=lambda: "users", path="/users", methods=["GET", "POST"], name="users"
        )

        route2 = RouteMetadata(
            func=lambda: "user", path="/users/{id}", methods=["GET"], name="user"
        )

        route3 = RouteMetadata(
            func=lambda: "user_posts", path="/users/{id}/posts", methods=["GET"], name="user_posts"
        )
    ...
    def test_find_websocket(self):
        ...
        ws3 = WebSocketMetadata(
            func=lambda ws: None, path="/chat/{room}/user/{user_id}", name="ws3"
        )
    ...
    def test_route_priority_first_match(self):
        ...
        generic_route = RouteMetadata(
            func=lambda: "generic", path="/items/{id}", methods=["GET"], name="generic"
        )

        # This could match /items/special but generic_route is added first
        special_route = RouteMetadata(
            func=lambda: "special", path="/items/special", methods=["GET"], name="special"
        )
