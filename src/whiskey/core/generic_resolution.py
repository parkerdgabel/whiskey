...
from .errors import TypeAnalysisError, ResolutionError
...
class GenericTypeResolver:
    ...
    def _score_candidate_compatibility(self, generic_type: Any, candidate: type, args: tuple) -> int:
    ...
    def _is_compatible_type(self, expected: Any, actual: Any) -> bool:
        ...
        try:
            ...
            if expected_origin and actual_origin:
    ...
    def _find_compatible_implementation(self, generic_type: Any, candidates: list[type]) -> type | None:
    ...
    def _is_implementation_compatible(self, target_generic: Any, impl_class: type, impl_base: Any) -> bool:
    ...
    def _can_bind_typevar(self, typevar: TypeVar, target_type: Any) -> bool:
        ...
        if typevar.__constraints__:
            if not any(self._is_compatible_type(target_type, constraint) 
    ...
    def analyze_generic_type(self, generic_type: Any) -> dict[str, Any]:
        ...
        if origin is not None:
            ...
            # Check if resolvable
            analysis['resolvable'] = len(analysis['concrete_implementations']) > 0 or \
                                   self._discover_implementation(generic_type) is not None
    ...
    def _is_abc_type(self, type_hint: Any) -> bool:
        ...
        try:
            ...
            return (inspect.isclass(type_hint) and 
                   issubclass(type_hint, abc.ABC) and
                   bool(getattr(type_hint, '__abstractmethods__', set())))
        except (TypeError, AttributeError):
            return False
    ...
    def infer_type_parameters(self, generic_class: type, concrete_class: type) -> dict[TypeVar, Any]:
...
class TypeParameterBinder:
    ...
    def bind(self, typevar: TypeVar, concrete_type: Any) -> None:
        ...
        if typevar.__bound__ and not self._is_subtype(concrete_type, typevar.__bound__):
            raise TypeAnalysisError(f"Type {concrete_type} is not compatible with bound {typevar.__bound__}")
        
        if typevar.__constraints__:
            if not any(self._is_subtype(concrete_type, constraint) 
                      for constraint in typevar.__constraints__):
                raise TypeAnalysisError(f"Type {concrete_type} does not satisfy constraints {typevar.__constraints__}")
