...
class ServiceDescriptor:
    ...
    def __post_init__(self):
        ...
        if not (isinstance(self.provider, type) or 
                callable(self.provider) or 
                hasattr(self.provider, '__class__')):
            raise ValueError(f"Provider must be type, callable, or instance, got {type(self.provider)}")
        
        # Auto-detect if provider is a factory function
        if (callable(self.provider) and 
            not isinstance(self.provider, type) and 
            not hasattr(self.provider, '__class__')):
