...
from typing import Any, get_args, get_origin, Union, Protocol, runtime_checkable
...
class TypeAnalyzer:
    ...
    BUILTIN_TYPES = {
        ...
        type(None), object, type, slice, range, enumerate, zip, reversed, filter, map,
        complex, memoryview, property, staticmethod, classmethod, super
    }
    
    # Standard library modules to avoid injecting from
    STDLIB_MODULES = {
        'builtins', 'sys', 'os', 'io', 'time', 'datetime', 'json', 'pickle',
        'pathlib', 'urllib', 'http', 'socket', 'threading', 'asyncio',
        'collections', 'itertools', 'functools', 'operator', 'math', 'random',
        'string', 're', 'struct', 'array', 'copy', 'pprint', 'reprlib',
        'enum', 'decimal', 'fractions', 'statistics', 'hashlib', 'hmac',
        'secrets', 'uuid', 'csv', 'xml', 'html', 'email', 'base64',
        'binascii', 'codecs', 'unicodedata', 'stringprep', 'readline',
        'rlcompleter', 'sqlite3', 'zlib', 'gzip', 'bz2', 'lzma', 'zipfile',
        'tarfile', 'tempfile', 'glob', 'fnmatch', 'shutil', 'logging'
    }
    ...
    def _analyze_type_hint(self, type_hint: Any) -> InjectResult:
        ...
        if self._is_stdlib_type(type_hint):
            return InjectResult(
                InjectDecision.NO,
                type_hint,
                "Standard library type (not registered)"
            )
        ...
        if inspect.isclass(type_hint):
            ...
            return InjectResult(
                InjectDecision.YES,
                type_hint,
                "Unregistered class (will attempt auto-creation)"
            )
    ...
    def _analyze_generic_type(self, type_hint: Any, origin: Any, args: tuple) -> InjectResult:
        ...
        if args and origin in (list, dict, tuple, set, frozenset):
            return InjectResult(
                InjectDecision.NO,
                type_hint,
                f"Generic type {origin.__name__} with parameters"
            )
        ...
        if origin is Literal:
            return InjectResult(
                InjectDecision.NO,
                type_hint,
                f"Literal types cannot be injected: {args}"
            )
    ...
    def _analyze_union_type(self, type_hint: Any, args: tuple) -> InjectResult:
        ...
        return InjectResult(
            InjectDecision.NO,
            type_hint,
            "Union with no registered members"
        )
    ...
    def _handle_forward_reference(self, type_str: str) -> InjectResult:
        ...
        return InjectResult(
            InjectDecision.ERROR,
            type_str,
            f"Cannot resolve forward reference: '{type_str}'"
        )
        ...
        try:
            ...
            if hasattr(type_hint, '__mro__'):
                return any(
                    ...
                    base.__module__ == 'typing' and 
                    base.__name__ == 'Protocol'
                    for base in type_hint.__mro__
                )
            
            # Check for typing.Protocol marker
            return (hasattr(type_hint, '_is_protocol') and 
                    type_hint._is_protocol)
    ...
    def _analyze_protocol_type(self, type_hint: Any, origin: Any) -> InjectResult:
        ...
        if self.registry and self.registry.has(type_hint):
            return InjectResult(
                InjectDecision.YES,
                type_hint,
                f"Registered Protocol implementation found"
            )
        ...
        return InjectResult(
            InjectDecision.NO,
            type_hint,
            "No Protocol implementation registered"
        )
    ...
    def _analyze_callable_type(self, type_hint: Any, args: tuple) -> InjectResult:
        ...
        if self.registry and self.registry.has(type_hint):
            return InjectResult(
                InjectDecision.YES,
                type_hint,
                "Registered Callable found"
            )
        
        return InjectResult(
            InjectDecision.NO,
            type_hint,
            "Callable types require explicit registration"
        )
    ...
    def _analyze_generic_service_type(self, type_hint: Any, origin: Any, args: tuple) -> InjectResult:
        ...
        if self.registry and self.registry.has(type_hint):
            return InjectResult(
                InjectDecision.YES,
                type_hint,
                f"Full generic type registered"
            )
        
        return InjectResult(
            InjectDecision.NO,
            type_hint,
            f"Generic type not registered: {origin}[{args}]"
        )
    ...
    def detect_circular_dependency(self, 
                                   ...
                                   path: list[type] = None) -> list[type] | None:
    ...
    def analyze_dependency_tree(self, root_type: type, max_depth: int = 10) -> dict[type, list[type]]:
