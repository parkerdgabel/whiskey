...
async def test_clean_strings():
    ...
    # Custom operations
    result = await clean_strings(record, fields=["city"], operations=["strip", "title"])
    assert result["city"] == "New York"
...
async def test_transform_chain():
    ...
    # Build chain
    chain.select(["id", "name", "email", "age"]).rename({"id": "user_id"}).filter(
        lambda r: r["age"] >= 18
    ).map(
        {
            "user_id": "user_id",
    ...
    record = {"id": 1, "name": "Alice", "email": "ALICE@EXAMPLE.COM", "age": 25, "extra": "ignored"}
