...
from whiskey import (
    Container, inject, singleton, scoped, provide, Inject,
    ComponentDiscoverer, ContainerInspector, discover_components
)
...
class NotificationService:
    ...
    def __init__(self,
                 email: Annotated[EmailNotifier, Inject()],
                 sms: Annotated[SMSNotifier, Inject()]):
        self.email = email
        self.sms = sms
...
async def demonstrate_basic_discovery():
    ...
    discoverable = discoverer.discover_module(
        __name__,
        decorator_name="_discoverable"
    )
    ...
    services = discoverer.discover_module(
        __name__,
        predicate=lambda cls: getattr(cls, '_layer', None) == 'service'
    )
