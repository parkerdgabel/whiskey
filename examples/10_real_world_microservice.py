...
@singleton(
    name="primary",
    condition=all_conditions(env_equals("DB_TYPE", "postgres"), env_exists("DATABASE_URL")),
)
...
@singleton(name="primary", condition=env_equals("DB_TYPE", "sqlite"))
...
class RedisCache:
    ...
    def health_check(self) -> dict:
        return {"status": "healthy", "type": "redis", "entries": len(self.data), "url": self.url}
...
class MemoryCache:
    ...
    def health_check(self) -> dict:
        return {"status": "healthy", "type": "memory", "entries": len(self.data)}
...
class Customer:
    ...
    def to_dict(self) -> dict:
        return {"id": self.id, "name": self.name, "email": self.email, "tier": self.tier}
...
class OrderProcessingService:
    ...
    def __init__(self, message_queue: Annotated[MessageQueue, Inject()]):
    ...
    async def process_order_request(self, customer_id: int, items: list[dict]) -> dict:
        ...
        try:
            ...
            customer_repo = await self.order_service.value.customer_repo.value.find_by_id(
                customer_id
            )
            if customer_repo:
            ...
            while True:
                ...
                try:
                    ...
                    if cache:
                        ...
                        print(
                            f"🏥 Cache health: {health['status']} ({health['type']}, {health['entries']} entries)"
                        )
                except Exception as e:
        ...
        async with app.lifespan():
            ...
            for i, order_request in enumerate(orders, 1):
                ...
                result = await order_processor.process_order_request(
                    order_request["customer_id"], order_request["items"]
                )
