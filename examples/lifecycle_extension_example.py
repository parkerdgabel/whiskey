...
class DatabasePool(Initializable, Disposable):
    ...
    async def health_check(self):
        ...
        if not self.connections:
            ...
            return {"status": "degraded", "message": f"Only {len(self.connections)} connections available"}
        else:
...
async def main():
    ...
    async def on_retry(data):
        print(f"⚠️  Retrying {data['type'].__name__}: attempt {data['attempt']}/{data['max_retries']}")
        
    @app.on("component.started")
    async def on_started(data):
        print(f"✅ {data['type'].__name__} started in {data['time']:.2f}s (attempt {data['attempt']})")
