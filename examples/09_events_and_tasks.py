...
from whiskey import Application, ApplicationConfig, inject, Inject
...
class UserService:
    ...
    async def update_user(self, user_id: int, **updates) -> dict:
        ...
        # Record in event store
        self.event_store.record_event("user.updated", {
            "user_id": user_id,
            "old_data": old_user,
    ...
    async def delete_user(self, user_id: int):
        ...
        # Record in event store
        self.event_store.record_event("user.deleted", {
            "user_id": user_id,
...
class OrderService:
    ...
    async def process_order(self, order_id: int) -> dict:
        ...
        self.event_store.record_event("order.processed", {
            "order_id": order_id,
            "order": order
        })
    ...
    async def ship_order(self, order_id: int, tracking_number: str) -> dict:
        ...
        # Emit shipping event
        self.event_store.record_event("order.shipped", {
            "order_id": order_id,
...
async def main():
    ...
    @app.on("user.created")
    @inject
    async def send_welcome_email(data: dict, notifier: NotificationService, metrics: MetricsCollector):
    ...
    @inject
    async def handle_order_created(data: dict, notifier: NotificationService, metrics: MetricsCollector):
        ...
        await notifier.send_sms(
            f"+1234567890",  # Mock phone number
    ...
    @inject
    async def handle_order_shipped(data: dict, notifier: NotificationService, metrics: MetricsCollector):
    ...
    @app.on("system.error")
    @inject
    async def handle_system_error(data: dict, metrics: MetricsCollector, notifier: NotificationService):
        ...
        await notifier.send_email(
            "admin@company.com",
            "System Error Alert",
            f"System error occurred: {error_msg}"
        )
    ...
    async def order_processing_task(order_service: OrderService):
        ...
        try:
            while True:
                ...
                # Find pending orders
                pending_orders = [
                    order for order in order_service.orders.values()
    ...
    try:
        async with app.lifespan():
            ...
            await user_service.update_user(alice["id"], name="Alice Smith-Jones", phone="+1234567890")
            ...
            print("⏳ Background tasks are processing orders, collecting metrics, and monitoring health...")
