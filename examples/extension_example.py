...
async def main():
    ...
    (
        ...
        .extend(
            lambda app: app.container.register_singleton(
    ...
    Whiskey(
        WhiskeyConfig(
            name="MyApp", extensions=[redis_extension, cache_extension, metrics_extension]
        )
    )
...
async def example_with_first_party():
    ...
    app = Whiskey().use(ai_extension, asgi_extension).use(redis_extension, cache_extension)
