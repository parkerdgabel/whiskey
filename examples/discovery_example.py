...
class NotificationService:
    ...
    def __init__(self, 
                 email: Annotated[EmailService, Inject()],
                 sms_gateway: str = "default-gateway"):  # Not injected
        self.email = email
...
async def discovery_demo():
    ...
    components = discover_components(
        ...
        predicate=lambda cls: (
            cls.__name__.endswith("Service") and 
            cls.__name__ != "EmailService"  # Exclude EmailService
        ),
        auto_register=True
    )
...
async def introspection_demo():
    ...
    if dependency_count:
        print("\n  Most depended upon:")
        for dep, count in sorted(dependency_count.items(), 
                                key=lambda x: x[1], 
                                reverse=True):
...
async def usage_demo():
    ...
    components = app.discover(
        __name__,
        decorator_name="_service",
        auto_register=True
    )
