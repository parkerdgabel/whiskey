...
from whiskey_ml.core.dataset import ArrayDataLoader, FileDataset
...
class TestModel:
    ...
    def test_model_summary(self):
        ...
        class TestModel(Model):
            def get_parameters(self):
                return {"layer1": np.zeros((100, 50)), "layer2": np.zeros((50, 10))}
...
class TestMLPipeline:
    ...
    async def test_pipeline_creation_and_config(self, app):
        ...
        class TestModel(Model):
            async def forward(self, inputs):
                return ModelOutput(predictions=np.random.randn(inputs["batch_size"], 2), loss=0.5)
