...
from whiskey_ml import ml_extension, MLPipeline, Dataset, Model, ModelOutput
from whiskey_ml.core.pipeline import PipelineState, PipelineConfig
...
class MockDataset(Dataset):
    ...
    def get_splits(self):
        ...
        train_loader = ArrayDataLoader(
            self.data[:70], self.labels[:70], batch_size=16
        )
        val_loader = ArrayDataLoader(
            self.data[70:85], self.labels[70:85], batch_size=16
        )
        test_loader = ArrayDataLoader(
            self.data[85:], self.labels[85:], batch_size=16
        )
...
class TestMLPipeline:
    ...
    async def test_pipeline_metrics_emission(self, context):
        ...
        context.app = Mock()
        context.app.emit = AsyncMock(side_effect=lambda event, data: emitted_events.append((event, data)))
