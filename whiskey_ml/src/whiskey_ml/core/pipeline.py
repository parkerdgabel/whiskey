...
from whiskey_ml.core.dataset import Dataset, DatasetConfig
        ...
        try:
            ...
            if hasattr(self.context, "app"):
                await self.context.app.emit(
                    "ml.pipeline.failed",
                    {"name": self.config.name, "error": str(e), "state": self.state.value},
                )
            ...
            await self.context.app.emit(
                "ml.pipeline.completed",
                {
                    ...
                    "result": {
                        ...
                        "final_metrics": result.training_history[-1]
