...
class TensorBoardHandler(MetricsHandler):
    ...
    async def handle_metrics(self, snapshot: MetricSnapshot) -> None:
        ...
        if "learning_rate" in snapshot.metrics:
            writer.add_scalar(
                "train/learning_rate", snapshot.metrics["learning_rate"], snapshot.epoch
            )
...
class WandBHandler(MetricsHandler):
    ...
    def _get_run(self, pipeline: str):
        ...
        if pipeline not in self.runs:
            run = self.wandb.init(
                project=self.project, entity=self.entity, name=pipeline, reinit=True
            )
    ...
    async def handle_metrics(self, snapshot: MetricSnapshot) -> None:
        ...
        metrics = {f"{snapshot.phase}/{k}": v for k, v in snapshot.metrics.items()}
...
class MLFlowHandler(MetricsHandler):
    ...
    async def handle_metrics(self, snapshot: MetricSnapshot) -> None:
        ...
        self._get_run(snapshot.pipeline)

        # Log metrics with phase prefix
        for metric_name, value in snapshot.metrics.items():
            self.mlflow.log_metric(f"{snapshot.phase}_{metric_name}", value, step=snapshot.epoch)
