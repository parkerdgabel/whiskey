...
from whiskey import Whiskey, component
...
class ConsoleMetricsHandler(MetricsHandler):
    ...
    async def handle_metrics(self, snapshot: MetricSnapshot) -> None:
        ...
        # Format metrics
        metrics_str = ", ".join(f"{name}: {value:.4f}" for name, value in snapshot.metrics.items())
...
class MetricsTracker:
    ...
    async def handle_metrics(self, snapshot: MetricSnapshot) -> None:
        ...
        await asyncio.gather(
            *[handler.handle_metrics(snapshot) for handler in self.handlers], return_exceptions=True
        )
    ...
    async def finalize_pipeline(self, pipeline: str) -> None:
        ...
        # Distribute to all handlers
        await asyncio.gather(
            *[handler.finalize(pipeline) for handler in self.handlers], return_exceptions=True
        )
