...
async def main():
    ...
    class VisualizedPipeline(MLPipeline):
        ...
        async def on_epoch_end(self, epoch: int, metrics: dict[str, float]) -> None:
            ...
            val_metrics = {
                "loss": max(0, val_loss),
                "accuracy": min(1.0, max(0, val_accuracy))
            }
            
            # Emit validation metrics
            if hasattr(self.context, "app"):
                await self.context.app.emit("ml.validation.metrics", {
    ...
    async with app:
        ...
        print(f"✓ Metrics logged to: ./ml_logs/tensorboard")
        print(f"✓ Run 'tensorboard --logdir ./ml_logs/tensorboard' to view")
