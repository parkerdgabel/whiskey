...
class ScopeAwareDataset(Dataset):
    ...
    async def load(self):
        ...
        decision = (
            0.5 * self.data[:, 0] + 0.3 * self.data[:, 1] ** 2 + 0.2 * np.sin(self.data[:, 2])
        )
...
class ScopeAwarePipeline(MLPipeline):
    ...
    async def on_epoch_end(self, epoch: int, metrics: dict[str, float]) -> None:
        ...
        try:
            ...
            experiment_logger = await self.container.resolve(ExperimentLogger, scope="experiment")
            await experiment_logger.log_run_result(f"{self.config.name}_epoch_{epoch}", metrics)

            # Use checkpointer (training-scoped)
            checkpointer = await self.container.resolve(ModelCheckpointer, scope="training")
            checkpoint_path = await checkpointer.save_checkpoint(self._model, epoch, metrics)
            print(f"   Checkpoint saved: {checkpoint_path}")

            # Get epoch metrics collector (epoch-scoped)
            metrics_collector = await self.container.resolve(EpochMetricsCollector, scope="epoch")
        ...
        self.epoch_results.append(
            {"epoch": epoch, "metrics": metrics, "timestamp": asyncio.get_event_loop().time()}
        )
        ...
        if ml_scopes:
            ...
            async with ml_scopes.evaluation(f"validation_epoch_{epoch}"):
                try:
                    ...
                    val_results = await validation_runner.run_validation(self._model, val_loader)
...
async def main():
    ...
    async with app:
        ...
        print("✓ All scopes were properly managed and cleaned up")
        ...
        async with app.ml_scopes.experiment("manual_experiment"):
            ...
            async with app.ml_scopes.training("manual_training"):
                await app.container.resolve(ModelCheckpointer, scope="training")
                    ...
                    async with app.ml_scopes.epoch(epoch):
                        ...
                        for batch_idx in range(5):
                            metrics_collector.collect_batch_metrics(
                                batch_idx, {"loss": 0.5 - (epoch * 0.1) + (batch_idx * 0.01)}
                            )
