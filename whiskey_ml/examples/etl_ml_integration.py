...
try:
    from whiskey_etl import etl_extension, Pipeline, validation_transform, ValidationMode
    HAS_ETL = True
except ImportError:
    HAS_ETL = False
...
async def main():
    ...
    if HAS_ETL:
        ...
        validator = (
            ...
            .field("age").required().type(int).range(0, 120).end_field()
            .field("income").required().type(float).range(0, None).end_field()
            .field("label").required().choices([0, 1]).end_field()
            .build()
        )
