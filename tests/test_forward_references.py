...
def test_simple_forward_reference():
    ...
    try:
        ...
        print("✓ SUCCESS: Forward reference resolved!")
...
def test_mutual_forward_references():
    ...
    try:
        app.resolve(ServiceX)
        print("✗ UNEXPECTED: Circular dependency not caught!")
        return False
    except Exception as e:
...
def test_forward_ref_with_namespace():
    ...
    try:
        ...
        print("✓ SUCCESS: Local forward reference resolved!")
...
def test_optional_forward_reference():
    ...
    class ConsumerWithOptional:
        def __init__(
            self, required: "OptionalService", optional: Optional["OptionalService"] = None
        ):
    ...
    try:
        ...
        print("✓ SUCCESS: Optional forward reference resolved!")
...
def test_forward_ref_not_registered():
    ...
    try:
        app.resolve(ServiceNeedingMissing)
        print("✗ UNEXPECTED: Should have failed for missing service")
        return False
    except Exception as e:
        print(f"✓ Expected failure: {type(e).__name__}: {e}")
        return True
...
def test_complex_forward_ref_chain():
    ...
    try:
        service_a = app.resolve(ServiceA)
        print("✓ SUCCESS: Forward reference chain resolved!")
