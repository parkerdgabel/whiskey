...
async def test_circular_dependencies():
    ...
    try:
        await app.resolve_async(ServiceA)
    ...
    try:
        await app.resolve_async(ServiceC)
...
async def test_scope_violations():
    ...
    try:
        with app.scope("session"), app.scope("request"):
            app.resolve(RequestService)
...
async def test_complex_types():
    ...
    try:
        await app.resolve_async(ComplexService)
    ...
    try:
        await app.resolve_async(OptionalService)
    ...
    try:
        ...
        if counter.count == expected_count:
            ...
            results.append(
                (
                    "FAIL",
                    f"Concurrent access failed: expected {expected_count}, got {counter.count}",
                )
            )
