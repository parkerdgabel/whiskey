...
class TestSingletonThreadSafety:
    ...
    def test_concurrent_singleton_creation_sync(self):
        ...
        for instance in results[1:]:
            assert instance is first_instance, f"Expected same instance, got different: {instance.connection_id} vs {first_instance.connection_id}"
        
        # Only one instance should have been created
        assert DatabaseConnection._instance_count == 1, f"Expected 1 instance, created {DatabaseConnection._instance_count}"
    ...
    def test_concurrent_singleton_with_dependencies_sync(self):
        ...
        for service in results[1:]:
            assert service.db is first_db, f"Expected same DB instance, got different"
        
        # Only one database instance should exist
        assert DatabaseConnection._instance_count == 1, f"Expected 1 DB instance, created {DatabaseConnection._instance_count}"
    ...
    async def test_mixed_sync_async_singleton_access(self):
        ...
        for instance in all_instances[1:]:
            assert instance is first_instance, "Mixed sync/async should return same singleton instance"
    ...
    def test_singleton_stress_test(self):
        ...
        # Only one instance should have been created
        assert DatabaseConnection._instance_count <= 1, f"Expected at most 1 instance, created {DatabaseConnection._instance_count}"
...
class TestRaceConditionScenarios:
    ...
    def test_initialization_race_condition(self):
        ...
        assert len(instance_ids) == 1, f"Race condition detected: created {len(instance_ids)} different instances: {instance_ids}"
