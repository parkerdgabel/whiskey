...
from whiskey import (
    Whiskey, Container, component, singleton, factory, scoped, inject,
    resolve, resolve_async, get_app
)
...
class ComplexService:
    def __init__(self, 
        ...
        list_type: List[str] = None,  # Collection type
        dict_type: Dict[str, Any] = None,  # Dict type
        callable_type: callable = None,  # Callable type
    ):
        self.repo = repo
...
async def stress_test():
    ...
    try:
        singleton_service = await app.resolve(SingletonService)
    ...
    if len(LeakyService.instances) > 100:
        results["failed"].append(f"Memory leak detected: {len(LeakyService.instances)} instances retained")
    else:
    ...
    if counter.count != 1000:
        results["passed"].append(f"✓ Race condition detected: count={counter.count} (expected 1000)")
    else:
    ...
    try:
        ...
        for i in range(0, 1000, 100):
            ...
            if cls:
                instance = await app.resolve(cls)
        ...
        async with app.container.scope("level1") as scope1:
            async with scope1.scope("level2") as scope2:
                async with scope2.scope("level3") as scope3:
                    # Deep nesting
                    service = await scope3.resolve(ThreadUnsafeService)
    ...
    try:
        failing = await app.resolve(FailingService)
        ...
        try:
            working = await app.resolve(OptionalDep)
...
async def performance_benchmark():
    ...
    for _ in range(10000):
        service = await app.resolve(ThreadUnsafeService)
    ...
    start = time.time()
    for _ in range(10000):
        result = await injected_function()
