...
async def test_complex_types():
    ...
    class ComplexService:
        def __init__(self, 
            ...
            list_type: List[str] = None,  # Collection type - should not be injected
            dict_type: Dict[str, Any] = None,  # Dict type - should not be injected
        ):
            self.repo = repo
...
async def test_memory_and_resources():
    ...
    ids = set(s.id for s in services)
    if len(ids) == 100:
...
async def test_edge_cases():
    ...
    try:
        unknown = await app.resolve("NonExistentService")
...
async def test_error_recovery():
    ...
    for i in range(3):
        try:
            service = await app.resolve(FailingService)
    ...
    try:
        partial = await app.resolve(PartiallyFailingService)
        ...
        if len(PartiallyFailingService.instances) > 0:
            results.append(("WARN", f"Resource leak: {len(PartiallyFailingService.instances)} partial instances"))
