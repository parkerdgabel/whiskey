...
class TestServiceBuilder:
    ...
    def test_with_metadata_multiple_calls(self, service_builder):
        ...
        assert result is service_builder
        assert service_builder._metadata == {
            "version": "1.0",
            "author": "test", 
            "priority": 5
        }
...
class TestApplicationBuilder:
    ...
    def test_fluent_application_building(self):
        ...
        builder = (ApplicationBuilder("test-app")
                  .singleton(DatabaseService, DatabaseService).build()
                  .service(CacheService, CacheService).as_singleton().build()
                  .factory("test_factory", test_factory).build()
    ...
    def test_conditional_registration(self):
        ...
        with patch.dict(os.environ, {"FEATURE_ENABLED": "yes"}):
            builder.service("feature_service", CacheService).when_env("FEATURE_ENABLED", "yes").build()
...
class TestCreateAppFunction:
    ...
    def test_create_app_fluent_usage(self):
        ...
        app = (create_app("test-app")
               .singleton(TestService, TestService).build()
               .service(DatabaseService, DatabaseService).build()
               .build_app())
...
class TestBuilderEdgeCases:
    ...
    def test_complex_conditions(self):
        ...
        def complex_condition():
            return (os.getenv("ENV") == "production" and 
                   os.getenv("FEATURE_ENABLED") == "true")
