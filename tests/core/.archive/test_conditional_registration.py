...
from whiskey import Container, factory, provide, scoped, singleton
...
class TestConditionalRegistration:
    ...
    def test_composite_conditions(self):
        ...
        # all_conditions
        assert (
            evaluate_condition(all_conditions(env_exists("VAR1"), env_equals("VAR1", "true")))
            is True
        )

        assert (
            evaluate_condition(all_conditions(env_exists("VAR1"), env_equals("VAR1", "false")))
            is False
        )
        ...
        assert (
            evaluate_condition(
                any_conditions(env_equals("VAR1", "false"), env_equals("VAR2", "false"))
            )
            is True
        )
        ...
        assert (
            evaluate_condition(
                any_conditions(env_equals("VAR1", "false"), env_equals("VAR2", "true"))
            )
            is False
        )

        # not_condition
        assert evaluate_condition(not_condition(env_equals("VAR1", "false"))) is True

        assert evaluate_condition(not_condition(env_equals("VAR1", "true"))) is False
    ...
    def test_multiple_conditional_registrations(self, container):
        ...
        @provide(condition=any_conditions(env_equals("ENV", "dev"), env_equals("ENV", "staging")))
        class NonProdService:
            pass

        @provide(
            condition=all_conditions(
                env_exists("FEATURE_A"), not_condition(env_truthy("FEATURE_B"))
            )
        )
