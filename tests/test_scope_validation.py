...
from whiskey.core.errors import CircularDependencyError, ConfigurationError, ResolutionError
...
class TestScopeValidation:
    ...
    def test_invalid_scope_dependency_singleton_to_scoped(self):
        ...
        with pytest.raises(
            ConfigurationError,
            match="Invalid scope dependency.*singleton.*cannot depend on.*request",
        ):
    ...
    def test_invalid_scope_dependency_singleton_to_transient(self):
        ...
        with pytest.raises(
            ConfigurationError,
            match="Invalid scope dependency.*singleton.*cannot depend on.*transient",
        ):
    ...
    def test_scope_hierarchy_validation(self):
        ...
        with pytest.raises(
            ConfigurationError, match="Invalid scope dependency.*session.*cannot depend on.*request"
        ):
