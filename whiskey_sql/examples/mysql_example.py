...
app.configure_database(
    url="mysql://root:password@localhost/whiskey_demo",
    pool_size=20,
    echo_queries=True
)
...
class ProductService:
    ...
    async def create_product(self, name: str, price: Decimal, stock: int) -> int:
        ...
        product_id = await self.db.fetch_val(
...
class OrderService:
    ...
    async def place_order(self, product_id: int, quantity: int) -> Order | None:
        ...
        async with self.db.transaction() as tx:
            ...
            product = await tx.fetch_one(
                self.product_queries.get_by_id,
                {"id": product_id}
            )
            ...
            # Update stock
            await tx.execute(
                self.product_queries.update_stock,
    ...
    async def get_daily_sales(self, start_date: datetime) -> list[dict]:
        ...
        return await self.db.fetch_all(
            self.order_queries.daily_sales,
