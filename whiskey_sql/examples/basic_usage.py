...
app.configure_database(
    url="postgresql://localhost/whiskey_example", pool_size=10, echo_queries=True
)
...
class UserService:
    ...
    async def create_user(self, name: str, email: str) -> User:
        ...
        existing = await self.db.fetch_one(self.queries.get_by_email, {"email": email})
        if existing:
            raise ValueError(f"User with email {email} already exists")

        # Create user
        return await self.db.fetch_one(self.queries.create, {"name": name, "email": email}, User)

    async def get_user(self, user_id: int) -> User | None:
        """Get user by ID."""
        return await self.db.fetch_one(self.queries.get_by_id, {"id": user_id}, User)

    async def list_users(self, limit: int = 10, offset: int = 0) -> list[User]:
        """List all users with pagination."""
        return await self.db.fetch_all(
            self.queries.list_all, {"limit": limit, "offset": offset}, User
        )

    async def update_user(self, user_id: int, name: str) -> User | None:
        """Update user name."""
        return await self.db.fetch_one(self.queries.update, {"id": user_id, "name": name}, User)

    async def delete_user(self, user_id: int) -> bool:
        """Delete a user."""
        status = await self.db.execute(self.queries.delete, {"id": user_id})
...
class PostService:
    ...
    async def create_post(
        self, user_id: int, title: str, content: str, published: bool = False
    ) -> Post:
        """Create a new post."""
        return await self.db.fetch_one(
            self.queries.create,
            {"user_id": user_id, "title": title, "content": content, "published": published},
            Post,
        )

    async def get_user_posts(self, user_id: int) -> list[Post]:
        """Get all posts for a user."""
        return await self.db.fetch_all(self.queries.get_user_posts, {"user_id": user_id}, Post)
...
@app.main
@inject
async def main(user_service: UserService, post_service: PostService, db: Database):
    ...
    user_count = await db.fetch_val(SQL("SELECT COUNT(*) FROM users"))
    print(f"Total users: {user_count}")

    # Get statistics
    stats = await db.fetch_row(SQL("SELECT COUNT(*) as user_count, MAX(id) as max_id FROM users"))
    if stats:
        count, max_id = stats
