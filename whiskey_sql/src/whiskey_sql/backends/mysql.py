...
try:
    ...
    raise ImportError(
        "aiomysql is required for MySQL support. Install with: pip install whiskey-sql[mysql]"
    ) from e

from whiskey_sql.core import SQL, ConnectionContext, Database, Transaction
...
async def create_pool(**config) -> MySQLDatabase:
    ...
    if parsed.hostname is None:
        raise DatabaseConnectionError(f"Invalid MySQL URL: {url}")
    ...
    try:
        ...
        raise DatabaseConnectionError(f"Failed to create MySQL pool: {e}") from e
...
class MySQLDatabase(Database):
    ...
    async def stream(
        ...
        async with (
            ...
            conn.cursor(aiomysql.cursors.SSDictCursor) as cursor,
        ):
            await cursor.execute(converted_query, converted_params)
