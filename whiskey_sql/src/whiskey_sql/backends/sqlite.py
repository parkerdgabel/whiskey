...
class SQLiteDatabase(Database):
    ...
    def _map_result(self, row: aiosqlite.Row | None, result_type: Type[T] | None) -> T | dict | None:
        ...
        if hasattr(row, 'keys'):
            ...
            # Fallback for older aiosqlite versions
            row_dict = dict(row)
    ...
    async def stream(
        ...
        try:
            ...
            raise QueryError(f"Stream query failed: {e}", str(query), params)
        finally:
            await self.pool.release(conn)
...
class SQLiteTransaction(Transaction):
    ...
    async def fetch_one(self, query: SQL, params: dict = None, result_type = None):
    ...
    async def execute(self, query: SQL, params: dict = None) -> str:
