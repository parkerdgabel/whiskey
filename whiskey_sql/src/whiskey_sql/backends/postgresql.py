...
async def create_pool(**config) -> PostgreSQLDatabase:
    ...
    try:
        ...
        raise DatabaseConnectionError(f"Failed to create PostgreSQL pool: {e}") from e
...
class PostgreSQLDatabase(Database):
    ...
    async def _ensure_type_codecs(self) -> None:
        ...
        async with self.pool.acquire() as conn:
            ...
            await conn.set_type_codec(
                "json", encoder=lambda v: v, decoder=lambda v: v, schema="pg_catalog"
            )
            await conn.set_type_codec(
    ...
    def _map_result(
        self, record: asyncpg.Record | None, result_type: type[T] | None
    ) -> T | dict | None:
