...
class TestPriorityQueue:
    ...
    async def test_priority_order(self):
        ...
        for priority, name in jobs:
            metadata = JobMetadata(func=lambda: None, name=name, priority=priority)
            job = Job(metadata)
            await queue.push(job)
    ...
    async def test_same_priority_fifo(self):
        ...
        jobs = []
        for i in range(3):
            metadata = JobMetadata(func=lambda: None, name=f"job{i}", priority=JobPriority.NORMAL)
            job = Job(metadata)
            jobs.append(job)
            await queue.push(job)
    ...
    async def test_peek_highest_priority(self):
        ...
        low_metadata = JobMetadata(func=lambda: None, name="low", priority=JobPriority.LOW)
        low_job = Job(low_metadata)
        await queue.push(low_job)

        # Add high priority
        high_metadata = JobMetadata(func=lambda: None, name="high", priority=JobPriority.HIGH)
...
class TestMultiQueue:
        ...
        for queue_name in ["emails", "uploads", "analytics"]:
            for i in range(2):
                metadata = JobMetadata(
                    func=lambda: None, name=f"{queue_name}_job{i}", queue=queue_name
                )
                job = Job(metadata)
    ...
    async def test_queue_factory(self):
        ...
        metadata1 = JobMetadata(
            func=lambda: None, name="low", queue="test", priority=JobPriority.LOW
        )
        metadata2 = JobMetadata(
            func=lambda: None, name="high", queue="test", priority=JobPriority.HIGH
        )
    ...
    async def test_clear_operations(self):
        ...
        for queue_name in ["q1", "q2"]:
            metadata = JobMetadata(func=lambda: None, name=f"{queue_name}_job", queue=queue_name)
            job = Job(metadata)
            await multi_queue.push(job)
