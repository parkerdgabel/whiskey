...
class ScheduledJob:
    ...
    def __init__(self, metadata: ScheduledJobMetadata):
        ...
        if metadata.cron:
            ...
            self._cron = croniter(metadata.cron, datetime.now(UTC))
        else:
            self._cron = None
...
class JobScheduler:
        ...
        for scheduled_job in self._scheduled_jobs.values():
            try:
                if scheduled_job.should_run():
                    ...
                    logger.info(
                        f"Queued scheduled job {job.job_id} ({job.name}) to queue '{job.queue}'"
                    )

            except Exception as e:
                logger.error(f"Error checking scheduled job {scheduled_job.metadata.name}: {e}")
            ...
            jobs.append(
                {
                    "name": name,
                    "schedule": (
                        scheduled_job.metadata.cron or f"every {scheduled_job.metadata.interval}s"
                    ),
    ...
    def get_stats(self) -> dict[str, Any]:
        ...
        """
        active_jobs = sum(1 for job in self._scheduled_jobs.values() if job._next_run is not None)
