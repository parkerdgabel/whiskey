...
def convert_value(value: Any, target_type: Type[T], path: str = "") -> T:
    ...
    try:
        ...
        # Handle list/List types
        elif origin in (list, type(Sequence)) or (hasattr(target_type, "__origin__") and target_type.__origin__ is list):
            if isinstance(value, str):
            ...
            if args:
                item_type = args[0]
                return [convert_value(item, item_type, f"{path}[{i}]") for i, item in enumerate(value)]
            return list(value)
        
        # Handle dict/Dict types
        elif origin in (dict, type(Mapping)) or (hasattr(target_type, "__origin__") and target_type.__origin__ is dict):
            ...
            if args and len(args) == 2:
                ...
                return {
                    convert_value(k, key_type, f"{path}.{k}"): convert_value(v, value_type, f"{path}.{k}")
...
def get_value_at_path(config: Union[Dict[str, Any], Any], path: str) -> Any:
    ...
    for i, part in enumerate(parts):
        ...
        if is_dataclass_instance(current):
            ...
            raise ConfigurationError(f"Cannot navigate into {type(current).__name__} at path '{current_path}'")
