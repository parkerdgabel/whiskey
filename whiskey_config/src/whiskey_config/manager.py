...
class ConfigurationManager:
    ...
    async def reload(self) -> bool:
        ...
        if changed:
            ...
            if self._schema:
                ...
                try:
                    self._config_instance = create_dataclass_from_dict(
                        self._schema, new_config_data
                    )
                except Exception as e:
                    # Rollback on error
    ...
    async def _notify_changes(self, old_data: dict[str, Any], new_data: dict[str, Any]) -> None:
        ...
        for path, (old_value, new_value) in changes.items():
            for callback in self._change_callbacks:
                try:
                    await callback({"path": path, "old_value": old_value, "new_value": new_value})
                except Exception as e:
    ...
    @classmethod
    def from_sources(
        cls, sources: list[Union[str, ConfigurationSource]], **kwargs
